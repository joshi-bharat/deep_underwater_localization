...
tf.summary.scalar('loss_ratio', l2_loss / (loss[0] + rloss[0]))

tf.summary.scalar('region_loss/total_loss', rloss[0])
tf.summary.scalar('region_loss/loss_x', rloss[1])
tf.summary.scalar('region_loss/loss_y', rloss[2])
tf.summary.scalar('region_loss/n_correct', rloss[5])
tf.summary.scalar('region_loss/loss_conf', rloss[3])
...
with tf.Session(config=config) as sess:
    ...
    for epoch in range(args.total_epoches):
        ...
        rloss_total, rloss_x, rloss_y, rloss_conf, rloss_nclass = AverageMeter(), AverageMeter(), AverageMeter(), AverageMeter(), AverageMeter()
        ...
        for i in trange(args.train_batch_num):
            _, summary, __y_pred, __y_true, __loss,  __region_loss, __labels, __global_step, __lr = sess.run(
                [train_op, merged, y_pred, y_true,  loss, rloss, slabels, global_step, learning_rate],
            ...
            rloss_nclass.update(__region_loss[5], args.batch_size)
            ...
            if __global_step % args.train_evaluation_step == 0 and __global_step > 0:
                ...
                info = "Epoch: {}, global_step: {} | loss: total: {:.2f}, x: {:.2f}, y: {:.2f}, conf: {:.2f}, n_class: {:.2f} | ".format(
                        epoch, int(__global_step), rloss_total.average, rloss_x.average, rloss_y.average, rloss_conf.average, rloss_nclass.average)
                print(info)
                logging.info(info)
