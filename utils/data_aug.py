...
def random_crop_with_constraints(bbox, size, slabel, min_scale=0.3, max_scale=1,
    ...
    for min_iou, max_iou in constraints:
        ...
        for _ in range(max_trial):
            ...
            if len(bbox) == 0:
                ...
                return bbox, (left, top, right-left, bottom-top), slabel
...
def resize_with_bbox(img, bbox, new_width, new_height, singleshot, interp=0, letterbox=False):
    '''
    ...
    if letterbox:
        ...
        for s in singleshot:
            ...
            for i in range(1, len(s)):
                if i % 2 == 0:
                    ss.append(s[i] * resize_ratio + dh)
                else:
                    ss.append(s[i] * resize_ratio + dw)
            slabels.append(ss)

        # print(slabels)
        return image_padded, bbox, slabels
    else:
        ori_height, ori_width = img.shape[:2]
        ...
        for s in singleshot:
            ...
            for i in range(1, len(s)):
                if i % 2 == 0:
                    ss.append(s[i] / ori_height * new_height)
                else:
                    ss.append(s[i] / ori_width * new_width )
            slabels.append(ss)

        return img, bbox, slabels
...
def random_expand(img, bbox, singleshot, max_ratio=2, fill=0, keep_ratio=True):
    '''
